...
export default function App() {
  ...
  // Detect iframe embed and tag <html> so CSS can fill the iframe seamlessly
  // (no fixed phone-frame width, no internal scroll, no outer padding).
  useEffect(() => {
    try {
      if (window.self !== window.top) {
        document.documentElement.dataset.embed = 'iframe'
      }
    } catch (e) {
      // cross-origin access throws; that itself means we are in an iframe
      document.documentElement.dataset.embed = 'iframe'
    }
  }, [])
  ...
  // Bridge for host-page integration (iframe embed).
  // Host posts { type: 'PREFILL', data: { flow, individual?, entity? } } to seed the form.
  // We do not validate origin here because the demo accepts any host; in production
  // restrict by event.origin to your own deployment.
  useEffect(() => {
    const handleMessage = (event) => {
      const msg = event.data
      if (!msg || typeof msg !== 'object') return
      if (msg.type !== 'PREFILL' || !msg.data) return

      const { flow, individual, entity, parties, entityDocs } = msg.data
      if ((flow === 'individual' || flow === 'joint') && individual) {
        dispatch({ type: 'SET_INDIVIDUAL_DATA', payload: individual })
      }
      if (flow === 'entity' && entity) {
        dispatch({ type: 'SET_ENTITY_DETAILS', payload: entity })
      }
      if (parties && Array.isArray(parties)) {
        dispatch({ type: 'CLEAR_PARTIES' })
        parties.forEach(party => dispatch({ type: 'ADD_PARTY', payload: party }))
      }
      if (entityDocs && typeof entityDocs === 'object') {
        Object.entries(entityDocs).forEach(([category, docs]) => {
          dispatch({ type: 'SET_ENTITY_DOC', category, payload: docs })
        })
      }
      setAgreedToTerms(true)
    }
    window.addEventListener('message', handleMessage)
    // Tell the host we're ready to receive PREFILL. The host's iframe.onLoad
    // fires before this useEffect runs, so a PREFILL posted then is lost.
    // This handshake is the reliable trigger.
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'EMBED_READY' }, '*')
      }
    } catch {
      // Cross-origin parent access can throw in edge cases — ignore.
    }
  ...
  return (
    <div className="phone-frame-wrapper">
      {/* Dev Tools panel — enabled for the Veriff Test Journey standalone app so
          we can jump between screens, switch flow/KYC variant, prefill data and
          theme while testing the Veriff SDK. (In the embedded build this is
          gated off because the host app drives flow selection via postMessage.) */}
      {true && (

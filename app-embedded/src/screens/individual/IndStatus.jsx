...
export default function IndStatus({ goTo, contextId }) {
  ...
  // Notify embedding host (parent window) when KYC completes successfully.
  // Posts to parent regardless of whether we're actually iframed; standalone runs
  // post to themselves which is harmless.
  useEffect(() => {
    if (phase !== 'success') return
    if (typeof window === 'undefined') return
    try {
      window.parent?.postMessage(
        { type: 'KYC_COMPLETE', flow: 'individual', contextId },
        '*'
      )
    } catch {
      // Cross-origin posting failure is non-fatal for the demo flow.
    }
  }, [phase, contextId])

...
export default function Screen11VerificationLinks({ formData, goNext }) {
  ...
  const entityDetails = formData.entityDetails
  ...
  const launchVerification = async (party, index) => {
    ...
    try {
      await openAlloyVerification(party, index, entityDetails, (result) => {
      ...
      // Don't crash the per-party loop on one failure — surface the error on this row.
      console.error('Alloy SDK error:', err)
      setStatus(index, { state: 'error', message: err.message || 'Verification service unavailable. Please contact support.' })
    }
  }
